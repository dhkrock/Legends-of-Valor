...
public class LovMap extends Grid {
    ...
    private void createOuterCell(CellType[][] map, List<StringBuilder> printableMap, int row, int col) {
        ...
        switch (map[cellRow][col]) {
            ...
            case BUSH:
            case INACCESSIBLE:
                printableMap.get(row).append(getOuterCellStr(getCell(cellRow, col).getIcon()));
                break;
        }
    }
        ...
        for(int row=0;row<LOV_MAP_SIZE_OF_CELLS;row++){
            for(int column=0;column<LOV_MAP_SIZE_OF_CELLS;column++){
                switch (cellTypes[row][column]) {
                    case NEXUS:
                        setNexusCell(row, column);
                        break;
                    case PLAIN:
                        setPlainCell(row, column);
                        break;
                    case KOULOU:
                        setKoulouCell(row, column);
                        break;
                    case CAVE:
                        setCaveCell(row, column);
                        break;
                    case BUSH:
                        setBushCell(row, column);
                        break;
                    case INACCESSIBLE:
                        setInaccessibleCell(row, column);
                        break;
                }
            }
        }
    ...
    public void checkAndUpdate(){
        ...
        for(int row=0;row<LOV_MAP_SIZE_OF_CELLS;row++){
            for (int col=0;col<LOV_MAP_SIZE_OF_CELLS;col++){
                ...
                if(heroCounter>0&&!getCell(row,col).isHasHero()){
        ...
        for(int row=0;row<LOV_MAP_SIZE_OF_CELLS;row++){
            for (int col=0;col<LOV_MAP_SIZE_OF_CELLS;col++) {
                ...
                if (nonFaintMonsterCounter > 0 && !getCell(row,col).isHasMonster()) {
    ...
    public boolean makeHeroMove(int destinationCellRow, int destinationCellColumn, Hero heroToMove){
        ...
        Cell destinationCell= getCell(destinationCellRow, destinationCellColumn);
        Cell startCell =getCell(heroToMove.getRow(),heroToMove.getCol());
